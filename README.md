...
- 등록 시간이 현 시간과 12시간이 차이나는 듯 하다 → RDS의 MySQL 서버 시간 설정 고려
...
8. 공고 수정 및 삭제
-  올바르게 동작을 하여 문제될 것은 없다

9. 로그인
- 전혀 문제가 없지만 디자인이 조금 무미건조한 느낌이 있다
...
16. 채팅방
- 웹 소켓 연결에 대한 로그가 콘솔에 길게 표시되기에 이것을 없애줄 필요가 있음
...
- 또한 사진과 같은 정보도 올릴 수 있게 기능을 확장해야 한다
...
- 최근 쪽지를 눌러서 쪽지함 바로가기를 누르면 자동으로 접히도록 만들어야 한다 (되기도 해서 왜 그런지 살펴보자)

24. 내 정보 페이지
- 수정할 것은 디자인 뿐이다.
- 그리고 수정하기를 눌렀을 때 왼쪽에 취소 버튼을, 오른쪽에 수정하기 버튼을 배치한다
